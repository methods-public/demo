...
const char *octopass_remove_quotes(char *s)
{
  if (s == NULL) {
    return;
  }
  ...
  //printf("%s:%d:%d\n", s, (int)strlen(s), i);
  //strncpy(d, &s[i], strlen(s) + f);
  char *res;
  memcpy(res, &s[i], strlen(s) - i);
  //res[strlen(res)] = '\0';
  return res;
}
...
void octopass_config_loading(struct config *con, char *filename)
{
  ...
  while (fgets(line, sizeof(line), file) != NULL) {
    ...
    char *raw_value = strtok_r(NULL, DELIM, &lasts);
    if (strlen(lasts) > 0) {
      char v[strlen(raw_value) + strlen(lasts)];
      sprintf(v, "%s %s", raw_value, lasts);
      raw_value = v;
    }
    char *value = strdup(octopass_remove_quotes(raw_value));
