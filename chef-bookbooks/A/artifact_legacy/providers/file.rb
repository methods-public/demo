...
def load_current_resource
  ...
  @current_resource = Chef::Resource::ArtifactLegacyFile.new(@new_resource.name)
  @current_resource
end
...
action :create do
  ...
  begin
    ...
    if retries > 0
      retries -= 1
      Chef::Log.warn "[artifact_legacy_file] Downloaded file checksum does not match the provided checksum. Retrying - #{retries} attempt(s) left."
      retry
    end
...
def checksum_valid?
  ...
  if Chef::Artifact.from_nexus?(file_location)
    ...
    if new_resource.checksum
      ...
      Chef::Log.debug "[artifact_legacy_file] No checksum provided for artifact_legacy_file, assuming checksum is valid."
      true
    end
  end
end
...
private
  ...
  # @return [void]
  def run_proc(name)
    execute_run_proc("artifact_legacy_file", new_resource, name)
  end
  ...
  # Returns the artifact_legacy_file cache path for cached checksums
  #
  # @return [String]
  def cache_path
    ::File.join(Chef::Config[:file_cache_path], "artifact_legacy_file")
  end
