...
[
  ...
  directory "aerospike-platform:#{dir_path}" do
    path dir_path
    mode '0755'
    recursive true
    action :create
...
execute 'initialize aerospike' do
  command './bin/aerospike init && touch .aerospike_initialized'
  user node[cookbook_name]['user']
  group node[cookbook_name]['group']
  cwd "#{node[cookbook_name]['prefix_root']}/aerospike"
  creates '.aerospike_initialized'
end
