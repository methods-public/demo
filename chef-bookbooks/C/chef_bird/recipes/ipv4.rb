service node['bird']['service_ipv4'] do
  action [:enable]
  supports status: true, restart: true, reload: true
end

directory ::File.dirname(node['bird']['include_path4']) do
  owner 'root'
  group 'root'
  mode 0o755
  recursive true
...
unless node['bird']['ipv4']['includes'].nil?
  node['bird']['ipv4']['includes'].each do |name, values|
    dpath = ::File.dirname(node['bird']['include_path4'])
...
template node['bird']['config_ipv4'] do
  source 'bird4.conf.erb'
  mode 0o644
  owner 'root'
  group 'root'
  notifies :start, "service[#{node['bird']['service_ipv4']}]", :immediately
  notifies :reload, "service[#{node['bird']['service_ipv4']}]", :immediately
end
