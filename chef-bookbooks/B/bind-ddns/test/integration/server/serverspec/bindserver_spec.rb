...
describe 'reverse of 172.17.0.2' do
  expected = '2.0.17.172.in-addr.arpa. 86400 IN PTR server-ddns.chef.kitchen.'
  it 'should be "server-ddns.chef.kitchen."' do
    expect(command(dig('172.17.0.2', true)).stdout.chomp).to eq(expected)
  end
end

describe file('/etc/resolv.conf') do
  its(:content) { should contain <<-CONFIG.gsub(/^ {4}/, '') }
...
describe file('/etc/named.conf') do
  ...
  its(:content) { should contain <<-ZONE.gsub(/^ {6}/, '') }
      zone chef.kitchen {
        type master;
  ...
  its(:content) { should contain <<-ZONE.gsub(/^ {6}/, '') }
      zone 0.17.172.in-addr.arpa {
        type master;
        file "dynamic/0.17.172.in-addr.arpa";
        allow-update { key reverse-key; };
        notify true;
      };
    ZONE
end
...
describe file('/etc/named-chef-kitchen.key') do
  its(:content) { should eq <<-KEY.gsub(/^ {4}/, '') }
    // Produced by Chef -- changes will be overwritten
...
describe file('/var/named/dynamic/0.17.172.in-addr.arpa.erb') do
  its(:content) { should contain '@ IN SOA ns.chef.kitchen. reversemaster \(' }
  its(:content) { should contain '  IN NS ns.chef.kitchen.' }
  its(:content) { should contain '2 IN PTR server-ddns.chef.kitchen.' }
end
...
cmd = 'named-checkzone 0.17.172.in-addr.arpa. '\
  '/var/named/dynamic/0.17.172.in-addr.arpa'
describe command(cmd) do
  its(:exit_status) { should eq 0 }
end
